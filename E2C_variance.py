...
class E2C(nn.Module):
    def __init__(self):
        ...
        self.B = nn.Parameter(torch.randn(2,7))
        self.o = nn.Parameter(torch.randn(2))
    ...
    def predict(self, x, u):
        ...
        nz_mean = torch.matmul(self.A , mean.unsqueeze(2)).squeeze(2) + torch.matmul(self.B ,u.unsqueeze(2)).squeeze(2) + self.o
        return nz_mean
    ...
    def forward(self, x, u, nx):
        ...
        rx = self.decode(z)
        C = torch.matmul(torch.matmul(self.A, torch.diag_embed(torch.exp(0.5 * log_var))),self.A.transpose(0, 1))
...
e2c.load_state_dict(torch.load('./saved_models/global_e2c.pth'))
...
def plot_durations(encoded_states_ndarray, predicted_next_states):
    plt.figure(2)
    plt.clf()
    plt.scatter(encoded_states_ndarray[:,0], encoded_states_ndarray[:,1], c=np.linspace(-5.0, 5.0, len(encoded_states_ndarray)))
    plt.pause(0.001)
    plt.figure(3)
    plt.clf()
    ...
    while not done:
        ...
        if done:
            if (e+1)%20 == 0 or e < 5:
                ...
                plt.scatter(encoded_states_ndarray[:,0], encoded_states_ndarray[:,1], c=np.linspace(-5.0, 5.0, len(encoded_states)))
                plt.savefig("using_Global_E2C{0}.png".format(e+1)) 
...
torch.save(e2c.state_dict(), './global_e2c.pth')
