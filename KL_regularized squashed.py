    ...
    def train_model(self):
        ...
        for c in range(self.update_times):
            ...
            with torch.no_grad():
                ...
                sampled_next_action = next_pi_distribution.sample()
                ...
                estimated_next_kl_div_term_target = torch.exp(-2 * (np.log(2) - sampled_next_action - F.softplus(-2 * sampled_next_action)).sum(axis = -1, keepdim = True)) * (log_prob_of_sampled_next_action - log_prob_of_sampled_next_action_default).sum(axis = -1, keepdim = True)
